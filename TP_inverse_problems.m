...
lambda_vec = logspace(1e-1,1e2,40);
err_vec = zeros(length(lambda_vec),1);
...
for i=1:length(lambda_vec)
        ...
        s_hat_mne = MNE(X,G,lambda_vec(i));
        err_vec(i) = immse(X,G*s_hat_mne);
        norm_s(i) = norm(s_hat_mne);

end
    
% L criterion
val = 352; % from the plot
[ d, ix ] = min( abs( err_vec- val) );
...
grid on;
dim = [.2 .5 .3 .3];
annotation('textbox',dim,'String',['Lambda min=' num2str(lambda_vec(ix))],'FitBoxToText','on');
print('images/MNE/L-curve.png','-dpng');
...
lambda_vec = linspace(1e-10,1e10,500);
...
for i=1:length(lambda_vec)
        ...
        s_hat_mne = MNE(X,G,lambda_vec(i));
        GCV(i) = immse(X,G*s_hat_mne)/(trace(eye()-G*transpose(G)*(G*transpose(G)+lambda_vec(i)*eye(sizeG(1)))))^2;
